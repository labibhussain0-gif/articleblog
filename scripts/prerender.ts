...
async function run() {
  ...
  for (const article of articles) {
    ...
    if (article.faq && Array.isArray(article.faq) && article.faq.length > 0) {
      ...
      let currentAnswerBlocks: string[] = [];
      for (const block of article.body) {
        if (block._type !== 'block') continue;
        
        // Optimized text extraction using reduce instead of map + join
        const text = block.children?.reduce((acc: string, c: any) => acc + (c.text || ''), '') || '';
        ...
        if (isHeading) {
          ...
          if (trimmedText.endsWith('?')) {
            if (currentQuestion && currentAnswerBlocks.length > 0) parsedFaqs.push({ question: currentQuestion, answer: currentAnswerBlocks.join('\n').trim() });
            currentQuestion = trimmedText;
            currentAnswerBlocks = [];
          } else if (currentQuestion) {
            if (currentAnswerBlocks.length > 0) parsedFaqs.push({ question: currentQuestion, answer: currentAnswerBlocks.join('\n').trim() });
            currentQuestion = null;
            currentAnswerBlocks = [];
          }
        } else if (currentQuestion && block.style === 'normal') {
          currentAnswerBlocks.push(text);
        }
      }
      if (currentQuestion && currentAnswerBlocks.length > 0) parsedFaqs.push({ question: currentQuestion, answer: currentAnswerBlocks.join('\n').trim() });
    }
