...
export default function HomePage() {
  ...
  const { mappedArticles, featuredArticle, latestArticles, noteworthyReads } = useMemo(() => {
    // Optimization: Memoize the mapping and slicing of articles to prevent unnecessary recalculation on every render.
  ...
  const categorySections = useMemo(() => {
    // Optimization: Pre-group articles into a hash map by category name (O(N))
    // to avoid O(N*M) complexity when filtering for each category section.
    const articlesByCategory = mappedArticles.reduce((acc: Record<string, any[]>, article) => {
    ...
    return categories.map(cat => {
      ...
      // Use O(1) hash map lookup instead of O(N) filter on the entire articles array
